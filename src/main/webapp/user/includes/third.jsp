...
<body class="m-2 p-6">
    <div class="container max-w-[1440px] bg-white shadow-lg rounded-lg pt-4 border-2 border-gray-300">
        ...
        <%
            ...
            String[] images = new String[0]; // Initialize an empty array

            try {
                conn = DBConnection.getConnection();
                // Corrected SQL query: Use t_id instead of images_id
                String query = "SELECT GROUP_CONCAT(image_path SEPARATOR ',') AS image_paths FROM images WHERE t_id = ?";
                ...
                pstmt.setInt(1, id); // Set the parameter value
                ...
                if (rs.next()) {	
                    ...
                    if (imagePaths != null && !imagePaths.trim().isEmpty()) {
                        ...
                        if (images.length > 0) {
                            firstImage = request.getContextPath() + "/" + images[0]; // Assign first image
                        }
                    }
                }
            } catch (Exception e) {
        ...
        %>
            ...
            <div class="w-md flex flex-col items-center pl-4 mt-4">
                <div class="overflow-y-auto flex flex-col space-y-4 pt-2 rounded-lg h-[600px] w-52 border-2 border-gray-300">
                    <% if (images.length > 0) { // Display only if images exist %>
